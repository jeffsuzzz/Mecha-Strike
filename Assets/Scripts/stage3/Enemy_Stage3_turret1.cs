...
public class Enemy_Stage3_turret1 : MonoBehaviour {
    ...
    void shoot()
    {
        ...
        if (Time.time > nextFire_side && shoot_once_side < 10)
        {
            ...
            Vector3 side_dir = target.transform.position - turret_side.transform.position;
        ...
        if (Time.time > nextFire_cent && shoot_once_cent < 1)
        {
            ...
            //Bullet.transform.Rotate(0, 90, 0);
            Vector3 cent_dir = target.transform.position - shootPos_cent.transform.position;
