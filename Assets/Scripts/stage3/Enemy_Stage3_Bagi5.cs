...
public class Enemy_Stage3_Bagi5 : MonoBehaviour {
	...
	void Update () {
        if (active)
        {
            if(forward == true )this.transform.position -= new Vector3(0, 0, moveSpeed * Time.deltaTime);
            else this.transform.position += new Vector3(0, 0, moveSpeed * Time.deltaTime);
            if (this.transform.position.z < 1350)
            {
                this.transform.position = new Vector3(this.transform.position.x, this.transform.position.y, 1350);
                forward = false;
            }
            if (this.transform.position.z > 1750)
            {
                this.transform.position = new Vector3(this.transform.position.x, this.transform.position.y, 1750);
                forward = true;
            }
            ...
            if (target.transform.position.z> 1120 && target.transform.position.x >-40) active = true;
        }
