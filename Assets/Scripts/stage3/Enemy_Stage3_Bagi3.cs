...
public class Enemy_Stage3_Bagi3 : MonoBehaviour {
	...
	void Update () {
        if (active)
        {
            ...
            if(forward) this.transform.position -= new Vector3(moveSpeed * Time.deltaTime, 0, 0);
            else this.transform.position += new Vector3(moveSpeed * Time.deltaTime, 0, 0);

            if (this.transform.position.x > moveMAX)
            {
                this.transform.position = new Vector3(moveMAX, this.transform.position.y, this.transform.position.z);
                forward = true;
            }
            if (this.transform.position.x < moveMIN)
            {
                this.transform.position = new Vector3(moveMIN, this.transform.position.y, this.transform.position.z);
                forward = false;
            }
