...
public class Enemy_Stage3_heli2 : MonoBehaviour {
	...
	void Update () {
        if (active)
        {
            ...
            float dis = target.transform.position.x-250 - this.transform.position.x;
            if (Mathf.Abs(dis) > 5)
            {
                if (dis > 0)
                {
                ...
                {
                    this.transform.position -= new Vector3(movespeed * Time.deltaTime, 0, 0);
                    if (transform.position.x < -220) transform.position = new Vector3(-220, transform.position.y, transform.position.z);
                }
            }

                if (moveright == true) transform.position -= new Vector3(0, 0, movespeed * Time.deltaTime);
            else transform.position += new Vector3(0, 0, movespeed * Time.deltaTime);

            if (transform.position.z > target.transform.position.z +100) moveright = true;
            if (transform.position.z < target.transform.position.z -100) moveright = false;
        ...
        {
            if (target.transform.position.z >1120 && target.transform.position.y>50 && GATE2 !=null && target.transform.position.z<3000) 
            active = true;
        }
    }
    void shoot()
